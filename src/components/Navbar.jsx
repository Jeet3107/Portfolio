  ...
  return (
    <>
      <nav className={`${base} ${scrolledStyle}`}>
        ...
        <a
          href="#hero"
          className="font-display font-extrabold text-lg tracking-widest text-accent"
        >
          Jeet Prajapati
        </a>
        ...
        <div className="flex items-center gap-4">
          ...
          >
            <span
              className={`block w-5 h-px transition-all duration-300 ${dark ? "bg-slate-100" : "bg-slate-800"} ${menuOpen ? "rotate-45 translate-y-2" : ""}`}
            />
            <span
              className={`block w-5 h-px transition-all duration-300 ${dark ? "bg-slate-100" : "bg-slate-800"} ${menuOpen ? "opacity-0" : ""}`}
            />
            <span
              className={`block w-5 h-px transition-all duration-300 ${dark ? "bg-slate-100" : "bg-slate-800"} ${menuOpen ? "-rotate-45 -translate-y-2" : ""}`}
            />
      ...
      {menuOpen && (
        <div
          className={`fixed inset-0 z-[490] pt-20 px-8 ${dark ? "bg-[#09090b]" : "bg-white"}`}
        >
