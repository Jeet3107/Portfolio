const PROJECTS = [
  {
    ...
    tags: ['React.js', 'Node.js', 'Express.js', 'MongoDB', 'JWT', 'Recharts', 'React Router v6'],
...
export default function Projects({ dark }) {
  ...
  const cardBg = dark ? 'bg-[#111116] hover:bg-[#13131a]' : 'bg-white hover:bg-slate-50 border border-slate-200'
  ...
  const linkColor = dark ? 'text-slate-500 hover:text-accent' : 'text-slate-400 hover:text-slate-900'

  return (
    <section id="projects" className={`px-8 md:px-20 py-24 border-t ${border}`}>
      <div className="section-label">Selected Work</div>
      <h2 className={`reveal font-display font-extrabold text-3xl md:text-5xl tracking-tight mb-10 ${heading}`}>
        Projects.
      </h2>

      <div className={`reveal flex flex-col gap-px ${dark ? 'bg-[#1c1c24]' : 'bg-slate-200'}`}>
        {PROJECTS.map((p, i) => (
          <div
            key={p.num}
          ...
          >
            ...
            <div className="p-8 md:p-10 grid md:grid-cols-[1fr_auto] gap-6 items-start">
              <div>
                <div className={`font-display font-extrabold text-5xl leading-none mb-4 ${numColor}`}>{p.num}</div>
                <h3 className={`font-display font-bold text-xl md:text-2xl tracking-tight mb-4 ${heading}`}>
                  {p.title}
                </h3>
                <p className={`${muted} text-sm leading-[1.85] mb-6 max-w-2xl`}>{p.desc}</p>
